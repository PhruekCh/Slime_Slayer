...
class BouncingSimulator:
    def __init__(self, num_balls, initial_speed, avatar_gif, cooldown_time):
        ...
        for i in range(3):  # Display 3 hearts
            ...
            heart.goto(self.canvas_width // 2 - 100 * (i + 1), self.canvas_height // 2 + 180)
    ...
    def initialize_balls(self, ball_count = 5, initial_speed = 0.7):
        ...
        for i in range(ball_count):
            x = random.uniform(-self.canvas_width / 2 + ball_radius, self.canvas_width / 2 - ball_radius)
            y = random.uniform(-self.canvas_height / 2 + ball_radius, self.canvas_height / 2 - ball_radius)
            angle = random.uniform(0, 360)
            vx = initial_speed * math.cos(math.radians(angle))
            vy = initial_speed * math.sin(math.radians(angle))
            gif_path = random.choice(["pics/slime_blue.gif", "pics/slime_red.gif", "pics/slime_green.gif", "pics/slime_purple.gif"])
            self.ball_list.append(ball.Ball(ball_radius, x, y, vx, vy, None, len(self.ball_list), gif_path))
    ...
    def clear_removed_ball_events(self, removed_balls):
        ...
        self.pq = [event for event in self.pq if event.a not in removed_balls and event.b not in removed_balls]
    ...
    def detect_attack(self, ball, x, y, direction, attack_range=100, attack_width=100):
        ...
        rotated_x = dx * math.cos(-direction_rad) - dy * math.sin(-direction_rad)
        rotated_y = dx * math.sin(-direction_rad) + dy * math.cos(-direction_rad)
    ...
    def detect_collision(self, ball):
        ...
        avatar_x, avatar_y = self.avatar.avatar.pos()
        distance = math.sqrt((ball.x - avatar_x) ** 2 + (ball.y - avatar_y) ** 2)
    ...
    def handle_collision(self, ball):
        ...
        if current_time - self.last_collision_time >= self.collision_cooldown and self.avatar_lives > 0:
            ...
            self.avatar_lives -= 1
            self.hearts[self.avatar_lives].shape("pics/black_heart.gif")  # Update heart display
    ...
    def pause_game(self, settle):
        ...
        if settle == "L":
            ...
            self.display_message("You won! Press 'R' to Restart, 'N' to go to Next Level")
    ...
    def restart_game(self):
        ...
        if self.is_paused:
            ...
            self.is_paused = False
            self.screen.clear()
            self.__init__(self.num_balls, 1.5, "pics/hutao.gif", 3)  # Reinitialize the simulator
            heapq.heappush(self.pq, my_event.Event(0, None, None))  # Add an initial event
            self.run()
        else:
    ...
    def start_next_level(self):
        ...
        if self.is_paused:
            print(f"Starting Level {self.current_level + 1}...")  # Debug message
    ...
    def update_level_text(self, level_number):
        ...
        if hasattr(self, 'level_text'):
            ...
            self.level_text.goto(0, self.canvas_height // 2 + 110)  # Position at the bottom

        self.level_text.write(f"Level {level_number}", align="center", font=("Times New Roman", 24, "bold"))
    ...
    def display_message(self, message):
        ...
        turtle.write(message, align="center", font=("Times New Roman", 24, "bold"))
    ...
    def run(self):
        ...
        # Bind key
        self.screen.listen()
        self.screen.onkeypress(lambda: self.on_key_press(lambda: self.avatar.attack(self.remove_balls)), "e")
        self.screen.onkeypress(self.restart_game, "r")  # Bind 'R' for restart during pause
...
if __name__ == "__main__":
    ...
    simulator = BouncingSimulator(num_balls, initial_speed, avatar_gif, cooldown_time)
    simulator.run()
